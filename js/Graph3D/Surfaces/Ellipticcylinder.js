Surfaces.prototype.ellipticcylinder = (count = 40, point = new Point(0, 0, 0), color1 = '#ff0000',color2 = '#00008B' , animation) => {
    let points = [];
    let edges = [];
    let polygons = [];
    const PI = Math.PI;
    ...
    for (let i = 0; i < points.length; i++) {
        ...
        if ((i + 1) >= count && i + count < points.length && (i + 1) % count === 0) {
            polygons.push(new Polygon([i, i - count + 1, i + 1, i + count], color2));
        }
    }
    return new Subject(
